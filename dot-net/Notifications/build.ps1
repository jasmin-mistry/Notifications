...
try {
	...
	if (-not $SkipBackend) {
		...
		# Run tests
		Invoke-TestCore -ProjectNameFilter "*IntegrationTests" -Config $config -CoverletExcludes "[*.*Tests?]*", "[*]Notifications.DataAccess.Migrations.*", "[*]Notifications.DataAccess.Mapping.MapperConfiguration" -Skip:$SkipTests 
		Invoke-TestCore -ProjectNameFilter "*UnitTest*" -Config $config -CoverletExcludes "[*.*Tests?]*", "[*]Notifications.DataAccess.Migrations.*", "[*]Notifications.DataAccess.Mapping.MapperConfiguration" -Skip:$SkipTests
